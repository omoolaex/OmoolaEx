...
import { AlertTriangle, HardDrive, CloudOff } from "lucide-react";
import React from "react";

export default function CloudSolutionChallenge() {
  return (
    <section
      id="challenge"
      ...
      <div className="relative max-w-6xl mx-auto px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="relative z-10 bg-[#071230] border border-white/10 rounded-3xl shadow-xl p-8 backdrop-blur-sm">
              ...
              <ul className="space-y-3 text-sm text-[#C9D8FF]">
                <li className="flex items-start gap-2">
                  <HardDrive className="w-4 h-4 text-[#00D1FF] mt-0.5" />
                  <span>Aging on-premise servers that demand constant maintenance</span>
                </li>
                <li className="flex items-start gap-2">
                  <CloudOff className="w-4 h-4 text-[#00D1FF] mt-0.5" />
                  <span>Systems that can’t scale with business growth</span>
                </li>
                <li className="flex items-start gap-2">
                  <AlertTriangle className="w-4 h-4 text-[#00D1FF] mt-0.5" />
                  <span>Inadequate backup or disconnected tools risking data loss</span>
                </li>
              </ul>
            </div>
          </div>
        ...
        >
          ...
          <p className="text-[#0E1A3C] leading-relaxed mb-4">
            Many businesses run on aging infrastructure—on-premise servers requiring constant
            maintenance, systems that can&apos;t scale with demand, inadequate backup solutions risking
            data loss, or disconnected tools making collaboration difficult. The costs add up:
            downtime, security vulnerabilities, and IT teams stretched thin managing infrastructure
            instead of driving innovation.
          </p>

          <p className="text-[#0E1A3C] leading-relaxed">
            Cloud solutions and modern infrastructure solve these problems, but migration requires
            careful planning, skilled execution, and continuous optimization to unlock the full
            benefits without disrupting business operations.
          </p>
          ...
          >
            Discover how OmoolaEx modernizes infrastructure with cloud innovation →
          </a>
        </motion.div>
      </div>
    </section>
